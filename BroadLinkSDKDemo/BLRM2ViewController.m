...
//    });
    ...
    if ([code intValue] == 0) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"学习模式开启失败！" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alertView show];
    }
...
//    });
    BLRM2StudyModel * rm2StudyModel = [BLRM2StudyModel studyModelWithArgument:_info];
    NSString * code = [rm2StudyModel rm2SendControlData:_rmCode];
    if ([code intValue] == 0) {
        NSLog(@"\n命令发送成功\n");
    } else {
        NSLog(@"\n命令发送失败\n");
...
//    [alertView show];
    if ([[[responseData objectFromJSONData] objectForKey:@"code"] intValue] == 0) {
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:@"已经操作了遥控器？" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alertView show];
        [self rm2StudyControlData];
    } else {
...
//rm2 enter study mode failed
-(void)enterStudyModeFailed
{

}

/*study control data*/
- (void)rm2StudyControlData
{
    while (!isGetData) {
        NSMutableDictionary *dic = [[NSMutableDictionary alloc] init];
        [dic setObject:[NSNumber numberWithInt:133] forKey:@"api_id"];
        ...
        if ([[[responseData objectFromJSONData] objectForKey:@"code"] intValue] == 0)
        {
            isGetData = YES;
            _rmCode = [[responseData objectFromJSONData] objectForKey:@"data"];
            [self performSelectorOnMainThread:@selector(checkCodeSuccess) withObject:nil waitUntilDone:NO];
        }
